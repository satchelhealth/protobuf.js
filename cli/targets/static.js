...
function buildServer(ref, service) {
  var fullName = service.fullName.substring(1) + "Server";
  push("");
  pushComment([
      "Constructs a new " + service.name + " server.",
...
function buildClient(ref, service) {
    ...
    push("function " + escapeName(service.name) + "Client(rpcImpl, requestDelimited, responseDelimited) {");
    ++indent;
    push("$protobuf.rpc.Service.call(this, rpcImpl, requestDelimited, responseDelimited);");
    --indent;
    push("}");
    push("");
    push("(" + escapeName(service.name) + "Client.prototype = Object.create($protobuf.rpc.Service.prototype)).constructor = " + escapeName(service.name) + "Client;");
    ...
    if (config.create) {
        ...
        pushComment([
            ...
            "@param {boolean} [requestDelimited=false] Whether requests are length-delimited",
            "@param {boolean} [responseDelimited=false] Whether responses are length-delimited",
            "@returns {" + escapeName(service.name) + "Client} RPC client. Useful where requests and/or responses are streamed."
        ]);
        push(escapeName(service.name) + "Client.create = function create(rpcImpl, requestDelimited, responseDelimited) {");
            ++indent;
    ...
    service.methodsArray.forEach(function(method) {
        ...
        push(escapeName(service.name) + "Client.prototype" + util.safeProp(lcName) + " = function " + escapeName(lcName) + "(request, callback) {");
            ++indent;
            push("return this.rpcCall(\"" + escapeName(method.name) + "\", $root" + method.resolvedRequestType.fullName + ", $root" + method.resolvedResponseType.fullName + ", request, callback);");
        ...
        pushComment([
            method.comment || "Calls " + method.name + ".",
            "@name " + escapeName(service.name) + "Client#" + lcName,
            "@function " + lcName,
            "@instance",
            //"@param {" + method.resolvedRequestType.fullName.substring(1) + "|Object} request " + method.resolvedRequestType.name + " message or plain object",
            "@param {" + exportName(method.resolvedRequestType, !config.forceMessage) + "} request " + method.resolvedRequestType.name + " message or plain object",
